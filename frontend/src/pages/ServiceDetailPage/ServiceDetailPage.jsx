...
export default function ServiceDetail() {
  ...
  function getClientMissingFields() {
    ...
    // Date/time are optional here — if not provided we'll pick a random available slot
  ...
  const handleSubmit = async (e) => {
    ...
    setSubmitting(true);
    try {
      // If date/time not chosen by user, pick a random available slot
      let chosenDate = selectedDate;
      let chosenTime = selectedTime;
      function pickRandomSlot(svc) {
        const dates = Array.isArray(svc.dates) && svc.dates.length ? svc.dates.slice() : [];
        if (dates.length === 0) {
          const today = new Date().toISOString().split("T")[0];
          return { date: today, time: "09:00 AM" };
        }

        // try to find any date that has slots
        const shuffled = dates.sort(() => Math.random() - 0.5);
        for (const d of shuffled) {
          const times = (svc.slots && svc.slots[d]) || [];
        ...
        // fallback: pick random date and synthetic time
        const d = dates[Math.floor(Math.random() * dates.length)];
        return { date: d, time: "09:00 AM" };
      }

      if (!selectedDate || !selectedTime) {
        const picked = pickRandomSlot(service);
        chosenDate = chosenDate || picked.date;
        chosenTime = chosenTime || picked.time;
        toast(
          `No date/time selected — assigned ${chosenDate} ${chosenTime} for booking.`,
          { duration: 3000 },
        );
      }
