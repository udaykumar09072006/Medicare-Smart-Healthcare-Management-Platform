...
function buildFallbackSchedule() {
  ...
  for (let i = 0; i < 5; i += 1) {
    ...
    fallback[key] = DEFAULT_SLOTS.slice();
  }
  return fallback;
}
...
function normalizeSchedule(schedule) {
  ...
  Object.entries(schedule).forEach(([key, value]) => {
    if (Array.isArray(value) && value.length > 0) {
      normalized[key] = value.map((slot) => String(slot).trim()).filter(Boolean);
    }
  });
...
export default function DoctorDetail() {
  ...
  useEffect(() => {
    if (!doctor) return;
    const success = Number(doctor.success);
    if (!Number.isFinite(success) || success <= 0) {
      const randomRate = Math.floor(Math.random() * 21) + 60; // 60-80
      setFailureThreshold(randomRate);
    }
  }, [doctor]);
  ...
  const schedule = useMemo(() => {
    const normalized = normalizeSchedule(doctor?.schedule);
    if (normalized) return normalized;
    return buildFallbackSchedule();
  }, [doctor]);

  const next7 = useMemo(() => getScheduleDates(schedule), [schedule]);
  const fee = Number(doctor?.fee ?? doctor?.fees ?? 0);
  const startedYear = doctor?.createdAt
    ? new Date(doctor.createdAt).getFullYear()
    : null;

  useEffect(() => {
    if (!selectedDate && next7.length > 0) {
  ...
  const handleBooking = async () => {
    ...
    const missing = [];
    const patientName = String(formData.name || "").trim();
    const mobileDigits = String(formData.mobile || "").replace(/\D/g, "");
    ...
    try {
      ...
      const checkoutUrl = body?.checkoutUrl || null;
      if (checkoutUrl) {
              ...
              <div className={doctorDetailStyles.statsGrid}>
                <div className={doctorDetailStyles.statBox}>
                  ...
                  <div className={doctorDetailStyles.statValue}>
                    {doctor.success && doctor.success !== "" ? doctor.success : `${failureThreshold}%`}
                  </div>
                ...
                <div className={doctorDetailStyles.statBox}>
                  <Shield
                    className={`${doctorDetailStyles.statIcon} ${doctorDetailStyles.shieldIcon || ""}`}
                  />
                  <div className={doctorDetailStyles.statValue}>
                    {startedYear ? `Since ${startedYear}` : "—"}
                  </div>
                  <div className={doctorDetailStyles.statLabel}>Practice Since</div>
                </div>
                <div className={doctorDetailStyles.statBox}>
                  <Users
